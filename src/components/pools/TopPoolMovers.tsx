import { GreyCard, ScrollableX } from 'components/Card'
...
const DataCard = ({ poolData }: { poolData: PoolData }) => {
  return (
    <Container to={'pools/' + poolData.poolId}>
      <Wrapper>
        <AutoColumn gap="sm">
          <RowFixed>
...
export default function TopPoolMovers() {
  ...
  return (
    <ScrollableX>
      {topVolume.map((entry) =>
        entry.data ? <DataCard key={'top-card-pool-' + entry.data.poolId} poolData={entry.data} /> : null
      )}
    </ScrollableX>
  )
}
