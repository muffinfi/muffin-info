...
import { useSavedPools, useSavedTiers, useSavedTokens } from 'state/user/hooks'
...
const Search = ({ ...rest }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  const [tiersShown, setTiersShown] = useState(3)
  ...
  const watchListTierData = useTierDatas(savedTiers)
  ...
  const tierForList = useMemo(
    () => (showWatchlist ? watchListTierData ?? [] : tiers.sort((p0, p1) => (p0.volumeUSD > p1.volumeUSD ? -1 : 1))),
    [tiers, showWatchlist, watchListTierData]
  )
      ...
      <Container>
        ...
        <Menu hide={!showMenu} ref={menuRef}>
          <AutoColumn gap="lg">
            ...
            {poolForList
              .filter((p) => !POOL_HIDE.includes(p.poolId))
              .slice(0, poolsShown)
              .map((p, i) => {
                return (
                  <HoverRowLink onClick={() => handleNav(networkPrefix(activeNetwork) + 'pools/' + p.poolId)} key={i}>
                    <ResponsiveGrid key={i}>
                      <RowFixed>
                        ...
                        <SavedIcon
                          ...
                          fill={savedPools.includes(p.poolId)}
                          onClick={(e) => {
                            e.stopPropagation()
                            addSavedPool(p.poolId)
                          }}
                        />
                      </RowFixed>
                      <HideSmall>
            ...
            {tierForList
              .filter(
                (x) =>
                  !!x && !(TOKEN_HIDE.includes(x.pool.token0.address) || TOKEN_HIDE.includes(x.pool.token1.address))
              )
              .slice(0, tiersShown)
              .map((t, i) => {
                return (
                  <HoverRowLink
                    key={i}
                    onClick={() =>
                      handleNav(networkPrefix(activeNetwork) + 'pools/' + t.poolId + '/tiers/' + t.tierId.toString())
                    }
                  >
                    <ResponsiveGrid>
                      <RowFixed>
                        <DoubleCurrencyLogo address0={t.pool.token0.address} address1={t.pool.token1.address} />
                        <TYPE.label ml="10px" style={{ whiteSpace: 'nowrap' }}>
                          <HoverInlineText
                            maxCharacters={12}
                            text={`${t.pool.token0.symbol} / ${t.pool.token1.symbol}`}
                          />
                        </TYPE.label>
                        <GreyBadge ml="10px">{feeTierPercent(t.feeTier)}</GreyBadge>
                        <SavedIcon
                          id="watchlist-icon"
                          size={'16px'}
                          style={{ marginLeft: '10px' }}
                          fill={savedTiers.includes(normalizeKey([t.poolId, t.tierId]))}
                          onClick={(e) => {
                            e.stopPropagation()
                            addSavedTier([t.poolId, t.tierId])
                          }}
                        />
                      </RowFixed>
                      <HideSmall>
            ...
            {tierForList.length === 0 ? (
              <TYPE.main>{showWatchlist ? 'Saved tiers will appear here' : 'No results'}</TYPE.main>
            ) : null}
            <HoverText
              onClick={() => {
                setTiersShown((prev) => prev + 5)
              }}
              hide={!(tierForList.length > 3 && tierForList.length >= tiersShown)}
            >
              See more...
            </HoverText>
          </AutoColumn>
